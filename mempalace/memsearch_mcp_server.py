#!/usr/bin/env python3
"""
MemSearch MCP Server — lightweight, auto-initializing memory for Agent Studio
=============================================================================

Usage (Agent Studio MCP config):
  {
    "mcpServers": {
      "memsearch": {
        "command": "uvx",
        "args": ["--from", "git+https://github.com/MemPalace/mempalace.git[memsearch]", "memsearch-mcp"],
        "env": {
...
async def write_memory(content: str, heading: str | None = None) -> dict[str, Any]:
    ...
    chunks_indexed = await mem.aindex_file(str(file_path))
...
async def search_memory(
    ...
    Args:
        ...
        source_date_prefix: Optional date filter (e.g. "2026-03-24") to restrict
                           results to a specific day.

    Returns:
        List of dicts with: content, source_file, heading, score, date.
    """
    mem = _get_mem()

    filter_expr = None
    if source_date_prefix:
        filter_expr = f'source_file like "%{source_date_prefix}%"'

    results = await mem.asearch(query, top_k=top_k, filter_expr=filter_expr)
...
async def reindex_memory() -> dict[str, int]:
    """
    Rebuild the Milvus vector index from all markdown files in the memory directory.
    ...
    files = list(_MEMORY_PATH.glob("*.md"))
    total_chunks = 0
    for f in files:
        chunks = await mem.aindex_file(str(f))
        total_chunks += chunks
